...
import { ref } from 'vue'
import { invoke, listen, type UnlistenFn } from '../api/client'
import type {
    TaskRecord,
...
export const useTaskStore = defineStore('tasks', () => {
    ...
    function setupListeners(): () => void {
        ...
        // 监听服务端推送的下载完成通知，用浏览器 Notification API 展示
        unlisteners.push(
            listen<{ title: string; body: string }>('download-notification', (event) => {
                try {
                    if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
                        new Notification(event.payload.title, { body: event.payload.body })
                    }
                } catch (e) {
                    console.warn('显示浏览器通知失败:', e)
                }
            })
        )
