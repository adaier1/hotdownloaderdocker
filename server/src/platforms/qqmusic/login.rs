...
/// - `Ok(())`: 保存成功。
/// - `Err(String)`: 错误信息。
async fn save_credentials(app: &AppCtx, creds: &LoginCredentials) -> Result<(), String> {
...
pub(crate) async fn get_login_credentials(app: &AppCtx) -> (Option<String>, Option<String>) {
    let settings_json = store_wrapper::load_string(app, "settings").unwrap_or_default();
...
pub(crate) async fn check_credential_expired(app: &AppCtx) -> Result<bool, String> {
...
/// 并更新到设置存储中。
pub(crate) async fn refresh_credential(app: &AppCtx) -> Result<LoginCredentials, String> {
...
async fn read_full_credentials(app: &AppCtx) -> Option<LoginCredentials> {
...
pub(crate) async fn create_qr_login(app: AppCtx) -> Result<String, String> {
...
/// - `Ok(())`: 清除成功。
/// - `Err(String)`: 错误信息。
pub(crate) async fn logout(app: AppCtx) -> Result<(), String> {
    let settings_json = store_wrapper::load_string(&app, "settings").unwrap_or_default();
    let mut settings: Value = serde_json::from_str(&settings_json).unwrap_or(json!({}));
...
pub(crate) async fn get_login_status(app: AppCtx) -> Result<String, String> {
    let settings_json = store_wrapper::load_string(&app, "settings").unwrap_or_default();
    let settings: Value = serde_json::from_str(&settings_json).unwrap_or(json!({}));
    let uin = settings
        .get("loginUin")
