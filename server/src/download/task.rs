...
pub async fn download_task(ctx: TaskContext, controller: TaskController, app_handle: AppCtx) -> bool {
    ...
    // 1. 构建最终保存路径（只需一次）
    let (_is_saf, download_dir, _saf_folder_uri) = if !ctx.save_path.is_empty() {
    ...
    'download: loop {
        ...
        // 打开/续传文件
        if file.is_none() {
            match open_download_file(&app_handle, &ctx.task_id, &download_dir, &mut downloaded).await
            {
                Some(f) => {
                    file = Some(f);
                    // 更新 final_path
    ...
    if completed_ok {
        ...
        if download_lrc_enabled {
            ...
            {
                let lrc_path = write_lrc_file(&app_handle, lrc, &download_dir).await;
        ...
        // 全部处理完成，发送下载完成事件
        progress::emit_completed(&app_handle, &ctx.task_id, &download_dir, None);
    }
